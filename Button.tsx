...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { className, variant = 'default', size = 'default', loading, children, disabled, asChild, ...props },
    ref
  ) => {
    const classes = cn(
    ...
    if (asChild && isValidElement(children)) {
      // Merge className into the child element and avoid rendering a native button
      const child = children as ReactElement<any>;
      return cloneElement(child, {
        className: cn(classes, child.props.className),
        // do not forward unknown DOM props like `variant`, `size`, `loading`, `asChild`
        ...props,
      });
    }
